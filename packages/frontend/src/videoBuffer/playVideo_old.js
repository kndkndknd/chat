...
export async function playVideo(buffer) {
    ...
    console.log("videoElement currentTime:", videoBufferState.videoElement.currentTime);
    ...
    console.log("videoElement readyState:", videoBufferState.videoElement.readyState);
    if (videoBufferState.videoElement.currentTime > 0 &&
    ...
    else if (videoBufferState.videoElement.ended ||
        videoBufferState.videoElement.paused) {
        ...
        const mime = mimeCandidates.find((m) => videoBufferState.videoElement.canPlayType(m) !== "") ?? "video/webm";
        // 旧URLを解放
        // if (currentObjectUrl) {
