...
export function showImage(url, position, receive_ctx = canvasElement.ctx) {
    //canvasSizing()
    ...
    try {
        ...
        image.onload = () => {
            ...
            if (position === undefined) {
                const hght = aspect > window.innerWidth / window.innerHeight
                    ? window.innerWidth / aspect
                    : window.innerHeight;
                const wdth = aspect > window.innerWidth / window.innerHeight
                    ? window.innerWidth
                ...
                receive_ctx.drawImage(image, position.left, position.top, position.width, position.height);
            }
        };
    }
    catch (error) {
