...
import { initVideo, initVideoStream, textPrint, erasePrint, showImage, emojiState, canvasSizing, } from "../canvasEvent";
import { initAudio, click, 
// stopCmd,
// recordReqFromServer,
gainChange,
// streamPlay,
 } from "../webaudio";
import { chatReq, recordReqFromServer, initAudioStream, streamPlay, } from "../stream";
...
socketState.socket.on("stringsFromServer", (data) => {
...
socketState.socket.on("cmdFromServer", (cmd) => {
    cmdFromServer(cmd);
    stringsClient = "";
});
socketState.socket.on("stopFromServer", (data) => {
    erasePrint();
...
socketState.socket.on("textFromServer", (data) => {
    erasePrint();
    textPrint(data.text);
    if (cinemaFlag) {
        setTimeout(() => {
...
socketState.socket.on("recordReqFromServer", (data) => {
...
// CHATのみ向けにする
socketState.socket.on("chatFromServer", (data) => {
...
// CHAT以外のSTREAM向け
socketState.socket.on("streamFromServer", (data) => {
...
socketState.socket.on("voiceFromServer", (data) => {
    // speechVoice(data);
});
socketState.socket.on("gainFromServer", (data) => {
    gainChange(data);
});
socketState.socket.on("windowReqFromServer", (data) => {
    window.open(data.URL, "_blank", "width=" +
        String(data.width) +
        ",height=" +
        String(data.height) +
        ",top=" +
...
socketState.socket.on("emojiFromServer", (data) => {
    textPrint(data.text);
    setTimeout(() => {
        erasePrint();
    }, 500);
...
socketState.socket.on("rtcConnectionFromServer", async (data) => {
    ...
    if (data.type === "offer") {
        ...
        if (answer) {
            ...
            socketState.socket.emit("rtcConnectionFromClient", JSON.stringify(webRtcState.peerConnection?.localDescription));
    ...
    if (SUPPORTS_MEDIA_DEVICES && navigator.mediaDevices.getUserMedia) {
        ...
        */
        ...
        await socketState.socket.emit("connectFromClient", {
            clientMode: window.location.pathname.includes("noStream") ||
        ...
        webRtcState.peerConnection.onicecandidate = (event) => {
            if (event.candidate) {
                socketState.socket.emit("rtcConnectionFromClient", JSON.stringify({
                    type: "candidate",
