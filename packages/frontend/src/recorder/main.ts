...
const app = document.querySelector<HTMLDivElement>("#wrapper");
...
const previewEl = document.querySelector<HTMLVideoElement>("#preview");
...
const sendChunk = async (chunk: Blob) => {
  ...
  try {
    ...
    console.error(error);
    updateStatus(
      "アップロード中にエラーが発生しました。録画を停止してください。"
    );
  ...
  try {
    ...
    mediaRecorder.addEventListener("stop", () => {
      uploadChain = uploadChain.then(async () => {
        await fetch(`${SERVER_URL}/api/stop`, { method: "POST" }).catch(
          (error) => {
            console.error(error);
          }
        );
