// mic-buffer-processor.ts
// 型宣言（TS用の最小宣言。プロジェクトでは @types/webaudioapi 等を併用してください）
declare const processorSampleRate: number;
...
interface ChatProcessorOptions {
  processorOptions?: {
    initialBufferLengthState?: number;
  };
}

class ChatProcessor extends AudioWorkletProcessor {
  private bufferLengthState: number;
