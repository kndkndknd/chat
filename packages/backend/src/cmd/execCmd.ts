...
export const execCmd = async (strings: string, id: string): Promise<void> => {
  if (Object.keys(cmdList).includes(strings)) {
    ...
    joinOrLeave(strings as "JOIN" | "LEAVE", id);
    ...
    stringEmit("SWITCH " + switchState, true);
