...
export function selectOtherClient(rooms:SocketIO.Namespace, source:string) {
  ...
  for(let id in rooms) {
    if(target === String(id)) targetID = id
  }
  return targetID
}
...
export function pickCmdTarget(idHsh:{}, cmd:{cmd:string, property?: string, value?: number, flag?: boolean}, room = "all"){
...
export function cmdSelect(strings:string, statusList:{}){
  let cmd: {cmd:string, overlay?: boolean} = {cmd:""};
  let nowFlag = false
