...
export function findFirstClusterOffset(buf: Buffer): number {
  return buf.indexOf(Uint8Array.from(videoBufferState.CLUSTER_ID));
}
...
export function startsWithEbmlHeader(buf: Buffer): boolean {
  console.log("buff", buf.length);
  if (buf.length < videoBufferState.EBML_ID.length) return false;
  return buf
    .subarray(0, (videoBufferState.EBML_ID as Buffer).length)
    .equals(Uint8Array.from(videoBufferState.EBML_ID));
}
